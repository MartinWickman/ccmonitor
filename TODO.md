...
- [x] **13. Separate plugin marketplace into dedicated repo** — Moved marketplace to `martinwickman/claude-plugins` so `/plugin marketplace add` only clones a tiny repo (3 plugin files + README) instead of the entire ccmonitor source. Deleted `.claude-plugin/marketplace.json` from ccmonitor. Updated README.md and install.sh to point to the new repo. The `plugin/` directory stays in ccmonitor for local dev use.
