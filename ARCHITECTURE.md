...
* Session file cleanup - The `SessionEnd` hook deletes its own session file. Both `SessionStart` and `SessionEnd` scan for session files with dead PIDs and remove them. This means stale files from crashes get cleaned up the next time any session starts or ends — no daemon, no cron, no manual cleanup needed.
